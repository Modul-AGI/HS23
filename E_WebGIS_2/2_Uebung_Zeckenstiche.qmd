...
Benutze zudem die Cell Options um euren Code sauber darzustellen. Nutzt dazu `echo` und `output` um unnötige Cell Codes und Outputs nicht auszugeben. Benenne zudem alle eingeklappten Code Cells mittels `code-summary` sinnvoll. Am Schluss sollte euer Output in etwa so aussehen:
