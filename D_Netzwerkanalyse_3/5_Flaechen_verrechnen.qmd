...
Öffnen Sie anschliessend das Tool `Rescale Raster` und skalieren Skalieren sie jeden der drei Rasterdatensätze jeweils so, dass Sie Werte von 0 bis 100 erhalten. Geben Sie dabei bei "New minimum value" 0 und bei "New maximum value" 100 ein.
...
Zum Abschluss clippen sie den Output mit dem Werkezeug `Clip raster by Mask Layer (GDAL)`. Speichern Sie den Output unter final_result.tif in ihrem Projektordner ab. Passen Sie die Symbolisierung entsprechend an.