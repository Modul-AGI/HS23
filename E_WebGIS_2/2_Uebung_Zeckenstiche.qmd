...
In dieser Übung wollen wir mit unseren orginalen und simulierten Zeckenstichen aus dem Block Programmieren eine interaktive Karte erstellen.
...
2. Um technische Probleme zu vermeinden, haben wir heute eine ZHAW-eigene Cloud Lösung vorbereitet. Es handelt sich dabei um eine *gehostete* Version von Jupyter Lab, auf welche ihr euch mit eurem ZHAW Benutzername und Passwort einloggen könnt: [statistik-ide.zhaw.ch](https://statistik-ide.zhaw.ch). Klickt auf `+ New Session` und wählt ein `Jupyter Lab` und gebt einen Session Namen ein. 

**Wer allerdings lieber lokal arbeiten möchte kann dies gerne tun, mit den Kentnissen aus Programmieren I - III sollte das gut möglich sein**. 

![](figures/jupyter_notebook_1.PNG)

3. Wenn die Session geladen ist, könnt ihr eure Übungdaten über den `Upload` Button hochladen. Öffnet anschliessend ein neues Jupyter Notebook und speichert es unter dem Namen `visualisierung_zeckenstiche.ipynb`.
...
Ladet zudem die drei Datensätze `wald.gpkg`, `zeckenstiche_original.gpkg` und `zeckenstiche_simuliert.gpkg` mit der `read_file` von Geopandas in euer Jupyter Notebook.
...
Schaut dafür im [User Guide von Folium](https://python-visualization.github.io/folium/latest/user_guide.html) und sucht nach den Layer Control und versucht diesen eurer `map` hinzuzufügen. Zum Schluss kannst du die `map` aufrufen, damit diese Dargestellt wird. Das finale Produkt sollte wie folgt aussehen:
...
Fügt ganz oben in eurem Notebook eine neue *raw* Cell ein und kopiert folgende Metadaten für Quarto. Dies bewirkt, dass euer Quarto Output ein Titel und Autor hat. Zudem sagen wir Quarto das wir mit Python3 arbeiten. Für unseren Ausgabeformat, welches HTML sein soll, werden alle Code Cells automatisch eingekappt, was das HTML viel besser lesbar macht.
...
- `code-fold`: Soll der Code eingeklappt werden.
...
Seht euch das PDF `Textvorlage_WebGIS2.pdf` an. Versucht nun die Struktur dieses PDF's möglich gleich in eurem Jupyter Notebook nachzubauen. Nutzt dafür die oben beschriebenen Methoden. Das Bild findet ihr ebenfalls in den Übungsdaten.

Benutze zudem die Cell Options um euren Code sauber darzustellen. Nutzt dazu `echo` und `output` um unnötige Cell Codes und Outputs nicht auszugeben. Benenne zudem alle eingeklappten Code Cells mittels `code-summary` sinnvoll. Am Schluss sollte nur noch eine Map am Ende des HTML dargestellt werden.
